...
class CongVanController extends Controller {
	...
	public function postThem(Request $request) {
		...
		if ($request->hasFile('taptindinhkem')) {
			...
			if (strlen($taptindinhkem) < 50) {
				$file->move("upload/", $taptindinhkem);
				$congvan->tentaptindinhkem = $taptindinhkem;
			} else {
				return redirect('admin/congvan/them')->with('loi', 'Tên tập tin đính kèm phải nhỏ hơn 60ký tự');
			}
	...
	public function postSua(Request $request, $id) {
		...
		if ($request->hasFile('taptindinhkem')) {
			...
			if (strlen($taptindinhkem) < 100) {
				...
				return redirect('admin/congvan/them')->with('loi', 'Tên tập tin đính kèm phải nhỏ hơn 100ký tự');
			}
